...
public class ActorDataDrawer(NameDicts _dicts, DictBNpcNames _bNpcNames) : IGameDataDrawer
{
    ...
    {
        DrawBNpcTable();
        DebugUtility.DrawNameTable("事件NPC", ref _eNpcFilter,      true, _dicts.ENpcs.Select(kvp => ((ulong)kvp.Key.Id, kvp.Value)));
        DebugUtility.DrawNameTable("伙伴", ref _companionFilter, true, _dicts.Companions.Select(kvp => ((ulong)kvp.Key.Id, kvp.Value)));
        DebugUtility.DrawNameTable("坐骑",     ref _mountFilter,     true, _dicts.Mounts.Select(kvp => ((ulong)kvp.Key.Id, kvp.Value)));
        DebugUtility.DrawNameTable("装饰",  ref _ornamentFilter,  true, _dicts.Ornaments.Select(kvp => ((ulong)kvp.Key.Id, kvp.Value)));
        DebugUtility.DrawNameTable("世界",     ref _worldFilter,     true, _dicts.Worlds.Select(kvp => ((ulong)kvp.Key.Id, kvp.Value)));
    }

    /// <summary> Draw a list of battle NPCs associated with names and base IDs. </summary>
    private void DrawBNpcTable()
    {
        using var tree = ImRaii.TreeNode("战斗NPC");
        if (!tree)
            return;

        var resetScroll = ImGui.InputTextWithHint("##filter", "筛选...", ref _bNpcFilter, 256);
